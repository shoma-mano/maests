...
const allowedMimeTypes = ["image/png", "image/jpeg", "image/jpg", "image/gif", "video/mp4"];
 ...
 */
export const addMedia = (mimeType: string, filePath: string) => {
  // Validate if the provided MIME type is in the list of allowed types
  if (!allowedMimeTypes.includes(mimeType.toLowerCase())) {
    throw new Error(`Unsupported MIME type. Allowed types are: ${allowedMimeTypes.join(", ")}`);
  }
...
if (import.meta.vitest) {
  ...
  it("should throw an error for an unsupported mimeType", () => {
    expect(() => addMedia("application/pdf", "../assets/test.pdf")).toThrowError(
